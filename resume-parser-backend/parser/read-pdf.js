 ...
 *
 * Accepts either a file URL (for browser) or a Buffer (for Node.js/backend).
 *
 ...
 * }
 */
const readPdf = async (input) => {
  let pdfFile;
  if (typeof input === "string") {
    pdfFile = await pdfjs.getDocument(input).promise;
  } else {
    pdfFile = await pdfjs.getDocument({ data: new Uint8Array(input) }).promise;
  }
  let textItems = [];
  ...
  for (let i = 1; i <= pdfFile.numPages; i++) {
    ...
    const pageTextItems = textContent.items.map((item) => {
      ...
      const fontName = fontObj && fontObj.name ? fontObj.name : pdfFontName;
